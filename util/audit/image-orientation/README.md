...
Report stored image orientation.

Mode: **read**. Operation: `orientation`.

Source extensions: `.png`, `.jpg`, `.jpeg`, `.webp`, `.tif`, `.tiff`, `.bmp`, `.gif`, `.avif`, `.heic`, `.heif`, `.jxl`, `.ppm`.

Run from the repository root:

```bash
bin/image-utils image-orientation --help
```
...
```text
usage: image-orientation [-h] [--config CONFIG] [--apply] [-n] [-j JOBS] [-q]
